...
Common pitfalls
---------------
- In the UI or API, always enter the container path (e.g. `/data/pdfs`), not the Windows path (e.g. `D:\...`). The bind mount maps `D:\...` on the host to `/data/pdfs` inside the container, and only the container path is visible to the app.
